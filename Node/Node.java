    ...
    private void updateNeighbours(String new_name, String new_ip) throws NodeAlreadyExistsException {
        ...
        if(getNumberOfNodesInNetwork() > 1) {
            ...
            if((new_hash < myNext  && new_hash > my_hash) || (myNext < my_hash && (my_hash < new_hash || myNext > new_hash))) {
                ...
                // update the files.
                manager.checkFilesNewNextNode(myNext);
                ...
                manager.checkFilesNewPreviousNode();
            }
        } else {
            ...
            manager.checkFilesNewNextNode(Integer.MAX_VALUE); //Give max integer so the check for new highest node is always false
            manager.initialize();
